...
import com.tracelink.appsec.watchtower.core.scan.code.pr.PRScanAgent;
...
public class PRScanningService extends AbstractScanningService {
	...
	public PRScanningService(@Autowired ApiFactoryService apiFactoryService,
	...
	public void doPullRequestScan(PullRequest pr)
		...
		PRScanAgent scanAgent = new PRScanAgent(pr)
				...
				.withScanners(scanRegistrationService.getScanners(CodeScanConfig.class))
	...
	private List<PullRequest> recoverByApi(Map<String, List<RepositoryEntity>> repoMap,
		...
		List<PullRequest> prs = new ArrayList<>();
		try {
			IScmApi api = (IScmApi) apiFactoryService.createApiForApiEntity(entity);
